...
def main(file):
    ...
    corrected_maps = Parallel(n_jobs=-1, prefer="processes")(
        delayed(align_patch_func)(shared_predicted_map, shared_predicted_neighborhood_indices, i)
        for i in range(n_points)
    )

    corrected_maps = np.array(corrected_maps)
    np.save(os.path.join(res_path, f'corrected_maps_{file}.npy'), corrected_maps)
    end = time.time()
